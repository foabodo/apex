...
def check_cuda_torch_binary_vs_bare_metal(cuda_dir):
    ...
    if bare_metal_version.major != torch_binary_version.major:
        raise RuntimeError(
            "Cuda extensions are being compiled with a version of Cuda ({}) that does ".format(bare_metal_version)
            + "not match the major version used to compile Pytorch binaries ({}). ".format(torch_binary_version)
        )
    else:
        if bare_metal_version.minor < torch_binary_version.minor:
            raise RuntimeError(
                "Cuda extensions are being compiled with a version of Cuda ({}) that does ".format(bare_metal_version)
                + "not match the major version used to compile Pytorch binaries ({}). ".format(torch_binary_version)
            ...
            print(
                "Warning: Cuda extensions are being compiled with a version of Cuda ({}) that does ".format(bare_metal_version)
                + "not match the major version used to compile Pytorch binaries ({}). ".format(torch_binary_version)
                + "In some cases, a minor-version mismatch will not cause later errors:  "
                "https://github.com/NVIDIA/apex/pull/323#discussion_r287021798.  "
                "You can try commenting out this check (at your own risk)."
